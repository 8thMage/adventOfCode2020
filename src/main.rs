    ...
    for (index, scanner) in scanners.iter().enumerate() {
        'big_loop: for (index2, second_scanner) in scanners.iter().enumerate() {
            ...
            for beacon in scanner {
                let mut first_beacons = HashSet::new();
                for second_beacon_first_scanner in scanner {
                    first_beacons.insert(*second_beacon_first_scanner - *beacon);
                }
                for orientation in &orientations {
                    for first_beacon_second_scanner in second_scanner {
                        let mut second_beacons = HashSet::new();
                        for second_beacon_second_scanner in second_scanner {
                            second_beacons.insert(orientation.apply(
                                *second_beacon_second_scanner - *first_beacon_second_scanner,
                            ));
                        }
                        let intersection = second_beacons.intersection(&first_beacons);
                        if intersection.count() >= 12 {
                            scanner_orientation_and_positions.insert(
                                ...
                                OrientationAndPosition {
                                    orientation: *orientation,
                                    position: *beacon
                                        - orientation.apply(*first_beacon_second_scanner),
                                },
                            );
                            continue 'big_loop;
                        }
    ...
    for i in 1..scanners.len() {
        ...
        for j in 1..scanners.len() {
            let dist = scanner_orientation_and_positions[&(0, i)]
                ...
                .v
                .iter()
                .zip(scanner_orientation_and_positions[&(0, j)].position.v.iter()).fold(0, |acc, (a, b)| acc + (a - b).abs());
            max_dist = max_dist.max(dist);
        }
    }
    println!();
